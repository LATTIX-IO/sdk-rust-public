...
SYMPHONY_SUPERPROJECT_ROOT ?= $(strip $(shell git rev-parse --show-superproject-working-tree))
SYMPHONY_COORDINATION_ROOT ?= $(if $(SYMPHONY_SUPERPROJECT_ROOT),$(abspath $(SYMPHONY_SUPERPROJECT_ROOT)),$(abspath $(CURDIR)))
SYMPHONY_ROOT ?= $(abspath $(SYMPHONY_COORDINATION_ROOT)/../symphony)
SYMPHONY_ELIXIR_ROOT ?= $(SYMPHONY_ROOT)/elixir
SYMPHONY_RUST_ROOT ?= $(SYMPHONY_ROOT)/rust
SYMPHONY_WORKFLOW ?= $(CURDIR)/WORKFLOW.md
SYMPHONY_DOTENV_FILE ?= $(or $(wildcard $(CURDIR)/.env.symphony.local),$(wildcard $(SYMPHONY_COORDINATION_ROOT)/.env.symphony.local))
...
OMNIROUTE_RUN_ROOT ?= $(abspath $(SYMPHONY_COORDINATION_ROOT)/../.omniroute)
