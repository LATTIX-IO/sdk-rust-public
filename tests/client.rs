...
use sdk_rust::{
    ArtifactProfile, Client, ProtectionOperation, ResourceDescriptor, SdkAuthConfigurationMode,
    SdkProofOfPossession, SdkProtectionPlanRequest, WorkloadDescriptor,
};
...
fn capabilities_sends_auth_and_identity_headers() {
    ...
    assert_eq!(
        response.auth_configuration.mode,
        SdkAuthConfigurationMode::OauthClientCredentials
    );
    assert_eq!(
        response.auth_configuration.proof_of_possession,
        SdkProofOfPossession::Mtls
    );
